...
describe("oauth-client-context schema (01)", function()
  ...
  it("declares plugin name and config field block", function()
    ...
    assert.is_not_nil(get_config_field("signing_private_key"))
  end)

  it("requires signing_private_key as signing source", function()
    local signing_private_key = get_config_field("signing_private_key")
    assert.are.equal(true, signing_private_key.required)
    assert.are.equal(true, signing_private_key.referenceable)
  end)

  it("allows optional signing_key_id as referenceable kid source", function()
    local signing_key_id = get_config_field("signing_key_id")
    assert.are.equal(false, signing_key_id.required)
    assert.are.equal(true, signing_key_id.referenceable)
  end)

  it("enforces supported algorithms and default", function()
    local signing_algorithm = get_config_field("signing_algorithm")
    assert.is_not_nil(signing_algorithm)
    assert.are.equal("RS256", signing_algorithm.default)
    assert.same({ "RS256", "ES256" }, signing_algorithm.one_of)
  end)
  ...
  it("sets expected defaults for header and key extraction options", function()
    ...
    assert.are.equal("string", signing_private_key.type)
  end)
